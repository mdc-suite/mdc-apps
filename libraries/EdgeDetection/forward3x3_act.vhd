...
architecture FSM of Forward3x3 is
    type t_enum1 is (WAIT_W,WAIT_H,WORK);
...
begin
  ...
  begin
    ...
    -- soi.rdy, state=WAIT_H / p_h=soi, state:=WORK, height:=p_h
      ...
      n_width <= width;
      en_width <= false;
    -- state=WORK, x = width + 1 && y = height + 1, outY.rdy / state:=WAIT_W, x:=0, y:=0, wr(outY,0)
    elsif state=WORK and (((x) = ((width) + (to_unsigned(1,16)))) AND ((y) = ((height) + (to_unsigned(1,16))))) and outY_full='0' then
      n_state <= WAIT_W;
      en_state <= true;
      ...
      n_width <= width;
      en_width <= false;
    -- state=WORK, y = height + 1, outY.rdy / state:=WORK, x:=x+1, wr(outY,0)
    elsif state=WORK and ((y) = ((height) + (to_unsigned(1,16)))) and outY_full='0' then
      n_state <= WORK;
      en_state <= true;
    ...
    -- state=WORK, y = 0, outY.rdy / state:=WORK, x:=x+1, wr(outY,0)
      ...
      n_width <= width;
      en_width <= false;
    -- state=WORK, x = width + 1, outY.rdy / state:=WORK, x:=0, y:=y+1, wr(outY,0)
    elsif state=WORK and ((x) = ((width) + (to_unsigned(1,16)))) and outY_full='0' then
      n_state <= WORK;
      en_state <= true;
      n_x <= to_unsigned(0,16);
      en_x <= true;
