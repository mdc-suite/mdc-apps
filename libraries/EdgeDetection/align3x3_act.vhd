...
-- from file main.cph, on 2019-05-09 at 13:12:41, by <unknown>
...
architecture FSM of Align3x3 is
    type t_enum4 is (WAIT_W,WAIT_H,WORK);
    ...
    signal state : t_enum4;
    signal n_state : t_enum4;
...
begin
  ...
  begin
    ...
    -- soi.rdy, state=WAIT_H / p_h=soi, state:=WORK, height:=p_h
      ...
      n_width <= width;
      en_width <= false;
    -- inY.rdy, state=WORK, x = width + 1 && y < 2 / p_drop=inY, state:=WORK, x:=0, y:=y+1
    elsif inY_empty='0' and state=WORK and (((x) = ((width) + (to_unsigned(1,16)))) AND ((y) < (to_unsigned(2,16)))) then
      ...
      n_width <= width;
      en_width <= false;
    -- inY.rdy, state=WORK, y < 2 / p_drop=inY, state:=WORK, x:=x+1
    elsif inY_empty='0' and state=WORK and ((y) < (to_unsigned(2,16))) then
      p_drop := from_std_logic_vector(inY,8);
      inY_rd <= '1';
      n_state <= WORK;
      en_state <= true;
      ...
      n_width <= width;
      en_width <= false;
    -- inY.rdy, state=WORK, x < 2 / p_drop=inY, state:=WORK, x:=x+1
    elsif inY_empty='0' and state=WORK and ((x) < (to_unsigned(2,16))) then
      p_drop := from_std_logic_vector(inY,8);
      inY_rd <= '1';
      n_state <= WORK;
      en_state <= true;
      ...
      n_width <= width;
      en_width <= false;
    -- inY.rdy, state=WORK, x = width + 1 && y = height + 1, outY.rdy / p_fwd=inY, state:=WAIT_W, x:=0, y:=0, wr(outY,p_fwd)
    elsif inY_empty='0' and state=WORK and (((x) = ((width) + (to_unsigned(1,16)))) AND ((y) = ((height) + (to_unsigned(1,16))))) and outY_full='0' then
      ...
      n_width <= width;
      en_width <= false;
    -- inY.rdy, state=WORK, x = width + 1, outY.rdy / p_fwd=inY, state:=WORK, x:=0, y:=y+1, wr(outY,p_fwd)
    elsif inY_empty='0' and state=WORK and ((x) = ((width) + (to_unsigned(1,16)))) and outY_full='0' then
      p_fwd := from_std_logic_vector(inY,8);
      inY_rd <= '1';
      n_state <= WORK;
      en_state <= true;
